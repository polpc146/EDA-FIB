 ...
 */
#define PLAYER_NAME Nini_squant
...
struct PLAYER_NAME : public Player {
  ...
  bool condiciones_cuadrante(Ant soldado, int i, int j) {
      
      if (soldado.type == Soldier) return true;
      if (me() == 0) {
  ...
  bool move_a_favor (Ant my_worker, Pos posicion, vector<vector<int>>& huecos_ocupados) {
      ...
      Cell movimiento;
              if (posicion.i < my_worker.pos.i){
                  ...
                  if (movimiento.id == -1 and movimiento.type == Soil and condiciones_cuadrante(my_worker, my_worker.pos.i -1, my_worker.pos.j)) {
            ...
            if (posicion.j < my_worker.pos.j){
                  ...
                  if (movimiento.id == -1 and movimiento.type == Soil and condiciones_cuadrante(my_worker, my_worker.pos.i, my_worker.pos.j -1)) {
  ...
  bool move_en_contra (Ant my_worker, Pos posicion, vector<vector<int>>& huecos_ocupados) {
      ...
      Cell movimiento;
              if (posicion.i < my_worker.pos.i){
                  ...
                  movimiento = cell(my_worker.pos.i +1, my_worker.pos.j);
                  if (movimiento.id == -1 and movimiento.type == Soil and condiciones_cuadrante(my_worker, my_worker.pos.i +1, my_worker.pos.j) and my_worker.pos.i +1 >= 0 and my_worker.pos.i +1 < board_rows() and my_worker.pos.j >= 0 and my_worker.pos.j < board_cols()) {
                      
                      if(move_dir (my_worker, Down, huecos_ocupados)) return true;
                }
                movimiento = cell(my_worker.pos.i, my_worker.pos.j +1);
                  if (movimiento.id == -1 and movimiento.type == Soil and condiciones_cuadrante(my_worker, my_worker.pos.i, my_worker.pos.j +1) and my_worker.pos.i >= 0 and my_worker.pos.i < board_rows() and my_worker.pos.j +1 >= 0 and my_worker.pos.j +1 < board_cols()) {
  ...
  bool coger (Pos posicion, set<Pos> comida_puesta) {
      
      if(comida_puesta.find(posicion) != comida_puesta.end()) return false;
      return true;
  }
  
  Pos buscar_bonus (Ant my_worker, set<Pos> comida_puesta) {
      
      int x = 0;
      if (my_worker.type == Queen) x = 1;
      int n = board_rows();
      int m = board_cols();
      ...
      while (not cola.empty()) {
          ...
          for (int k = 0; k < 4; ++k) {
              ...
              if (ii >= 0 and ii < n and jj >= 0 and jj < m and visitados[ii][jj] == false and casilla.type == Soil) {
                  ...
                  Pos posicion(ii,jj);
                  if (casilla.bonus != None and (x == 1 or coger(posicion, comida_puesta))) return posicion;
                  cola.push({ii,jj});
  ...
  void bfs_trabajadoras (int id_worker, vector<vector<int>>& huecos_ocupados, set<Pos> comida_puesta) {
    ...
    //LA REINA ESTA CERCA
      ...
      if (my_worker.bonus == None and my_worker.life > 1) {
          ...
          Pos pos_bonus = buscar_bonus(my_worker, comida_puesta);
          Pos vacio(-1,-1);
          if (pos_bonus != vacio) {
  ...
  bool puedo_comer (Ant my_queen, vector<vector<int>>& huecos_ocupados, set<Pos> comida_puesta) {
    ...
    if (comida.id == -1 and comida.bonus != None) {
                ...
                move (my_queen.id,Up);
                comida_puesta.erase(my_queen.pos + Up);
                return true;
    }
    comida = cell(my_queen.pos.i +1, my_queen.pos.j);
    if (comida.id == -1 and comida.bonus != None) {
                ...
                comida_puesta.erase(my_queen.pos + Down);
                return true;
    }
    ...
    if (comida.id == -1 and comida.bonus != None) {
                ...
                comida_puesta.erase(my_queen.pos + Left);
                return true;
    }
    comida = cell(my_queen.pos.i, my_queen.pos.j +1);
    if (comida.id == -1 and comida.bonus != None) {
                ...
                comida_puesta.erase(my_queen.pos + Right);
                return true;
    }
    return false;
  }
  ...
  void comidas (Pos posicion, set<Pos> comida_puesta) {
      
      if (cell(posicion + Up).bonus != None) comida_puesta.insert(posicion + Up);
      if (cell(posicion + Down).bonus != None) comida_puesta.insert(posicion + Down);
      if (cell(posicion + Right).bonus != None) comida_puesta.insert(posicion + Right);
      if (cell(posicion + Left).bonus != None) comida_puesta.insert(posicion + Left);
      if (cell(posicion + Up + Up).bonus != None) comida_puesta.insert(posicion + Up + Up);
      if (cell(posicion + Up + Left).bonus != None) comida_puesta.insert(posicion + Up + Left);
      if (cell(posicion + Up + Right).bonus != None) comida_puesta.insert(posicion + Up + Right);
      if (cell(posicion + Down + Down).bonus != None) comida_puesta.insert(posicion + Down + Down);
      if (cell(posicion + Down + Right).bonus != None) comida_puesta.insert(posicion + Down + Right);
      if (cell(posicion + Down + Left).bonus != None) comida_puesta.insert(posicion + Down + Left);
      if (cell(posicion + Right + Right).bonus != None) comida_puesta.insert(posicion + Right + Right);
      if (cell(posicion + Left + Left).bonus != None) comida_puesta.insert(posicion + Left + Left);
      if (cell(posicion + Up + Up + Up).bonus != None) comida_puesta.insert(posicion + Up + Up + Up);
      if (cell(posicion + Up + Up + Right).bonus != None) comida_puesta.insert(posicion + Up + Up + Right);
      if (cell(posicion + Up + Up + Left).bonus != None) comida_puesta.insert(posicion + Up + Up + Left);
      if (cell(posicion + Down + Down + Down).bonus != None) comida_puesta.insert(posicion + Down + Down + Down);
      if (cell(posicion + Down + Down + Right).bonus != None) comida_puesta.insert(posicion + Down + Down + Right);
      if (cell(posicion + Down + Down + Left).bonus != None) comida_puesta.insert(posicion + Down + Down + Left);
      if (cell(posicion + Left + Left + Left).bonus != None) comida_puesta.insert(posicion + Left + Left + Left);
      if (cell(posicion + Left + Left + Up).bonus != None) comida_puesta.insert(posicion + Left + Left + Up);
      if (cell(posicion + Left + Left + Down).bonus != None) comida_puesta.insert(posicion + Left + Left + Down);
      if (cell(posicion + Right + Right + Right).bonus != None) comida_puesta.insert(posicion + Right + Right + Right);
      if (cell(posicion + Right + Right + Up).bonus != None) comida_puesta.insert(posicion + Right + Right + Up);
      if (cell(posicion + Right + Right + Down).bonus != None) comida_puesta.insert(posicion + Right + Right + Down);
  }
  
  void bfs_reina (int id_queen, vector<vector<int>>& huecos_ocupados, set<Pos> comida_puesta) {
      
      Ant my_queen = ant(id_queen);
      comidas(my_queen.pos, comida_puesta);
      //LA REINA SE PUEDE MOVER
      if (round() % queen_period() == 0) {
          
          if (puedo_comer(my_queen, huecos_ocupados, comida_puesta)) return;
        ...
        if (pos_comida != vacio) {
          ...
          Pos pos_bonus = buscar_bonus(my_queen, comida_puesta);
          if (pos_bonus != vacio) {
     ...
     if (round() > 24) {
        ...
        if (mis_soldados.size() < 3 and carbohidratos >= soldier_carbo() and proteinas >= soldier_prote() and lipidos >= soldier_lipid()) {
            ...
            Cell huevo;
                  ...
                  if (huevo.id == -1 and huevo.type == Soil and condiciones_cuadrante(my_queen, my_queen.pos.i -1, my_queen.pos.j) and my_queen.pos.i -1 >= 0 and my_queen.pos.i -1 < board_rows() and my_queen.pos.j >= 0 and my_queen.pos.j < board_cols()) {
                  ...
                  if (huevo.id == -1 and huevo.type == Soil and condiciones_cuadrante(my_queen, my_queen.pos.i, my_queen.pos.j -1) and my_queen.pos.i >= 0 and my_queen.pos.i < board_rows() and my_queen.pos.j -1 >= 0 and my_queen.pos.j -1 < board_cols()) {
  ...
  virtual void play () {
      ...
      set<Pos> comida_puesta;
      vector<int> reina = queens(me());
      bfs_reina(reina[0], huecos_ocupados, comida_puesta);
      ...
      for (int i = 0; i < (int)trabajadoras.size(); i++) bfs_trabajadoras(trabajadoras[i], huecos_ocupados, comida_puesta);
  }
